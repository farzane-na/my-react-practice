...
export default function ProductInCart({ id , image, name , price , off , onRemove,count,color}) {
    ...
    <div className="w-full flex items-center justify-between border border-gray-200 rounded-2xl p-4">
      <div className="flex items-center">
        ...
        <div className="flex justify-center items-center ml-5 border border-gray-200 rounded px-1">
          {productCount == 1 ? (
            <CiTrash className="w-4 h-4 text-blue-950 cursor-pointer" onClick={()=>onRemove(id)} />
          ) : (
            <HiMinus className="w-4 h-4 text-blue-950 cursor-pointer" onClick={minusProduct} />
          )}
